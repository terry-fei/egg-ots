...
function createClient(config, app) {
  ...
  client.get = async (tableName, pk, columns = []) => {
    ...
    const { primaryKey, attributes } = res.row;
  ...
  client.put = async (tableName, pk, attrs) => {
    const pkArr = Object.keys(pk).map((key) => {
      if (typeof pk[key] === 'number') {
        return { [key]: TableStore.Long.fromNumber(pk[key]) };
      }
  ...
  client.update = async (tableName, pk, attrs) => {
    const pkArr = Object.keys(pk).map((key) => {
      if (typeof pk[key] === 'number') {
        return { [key]: TableStore.Long.fromNumber(pk[key]) };
      }
  ...
  client.delete = async (tableName, pk) => {
    const pkArr = Object.keys(pk).map((key) => {
      if (typeof pk[key] === 'number') {
        return { [key]: TableStore.Long.fromNumber(pk[key]) };
      }
  ...
  client.range = async (tableName, startPk, endPk, limit) => {
